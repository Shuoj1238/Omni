...
class OnPolicyBuffer(BaseBuffer):  # pylint: disable=too-many-instance-attributes
    ...
    def store(self, **data: torch.Tensor) -> None:
        ...
        # 新增下面这一行,将cost数据添加到_step_costs中
        if len(self._step_costs) == 0 or len(self._step_costs[-1]) == self.path_start_idx:
            self._step_costs.append([])  # 如果是新的轨迹,则添加一个新的空列表
        self._step_costs[-1].append(data['cost'].item())  # 将cost添加到当前轨迹的列表中
        self.ptr += 1
    ...
    def finish_path(
        ...
        step_costs_path = os.path.join(os.path.expanduser("~"), r"C:\Users\28906\Desktop\cl2", "step_costs.npy")
        np.save(step_costs_path, self._step_costs)
        
        self._step_costs.clear()  # 清空_step_costs,为下一个trajectory做准备
